# Function that computes EHCP at a specific time_pass

ehcp_pass <- function(play_id, play_df, time_snap_orig, time_pass_orig, time_play_orig, receiver_id_orig, n_samples = 100, n_bart_samples = 1000){
  receiver_list <- 
    play_df %>%
    filter( (possession == 1 & time == time_snap_orig & Position %in% c("WR", "TE", "RB", "FB")) | (possession == 1 & time == time_snap_orig & nflId == receiver_id_orig)) %>%
    pull(nflId)
  time_snap <- time_snap_orig
  time_play <- time_play_orig
  #time_pass <- time_pass_orig
  ...
  for(receiver_id in receiver_list){
    ...
    if(length(index) > 0){
      ...
      capture.output(phat_1 <- rowMeans(predict.lbart(bart_fit1, newdata = counter_df)$prob.test), file = "NUL")
      capture.output(phat_2 <- rowMeans(predict.lbart(bart_fit2, newdata = counter_df)$prob.test), file = "NUL")
