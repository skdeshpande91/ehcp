assess_plays_receiver <- function(game_id){
  ...
  tmp_results[,"outcome"] <- rep(NA, times = n_plays)
  tmp_results[,"binary_outcome"] <- rep(NA, times = n_plays)
  ...
  for(play_ix in 1:n_plays){
    ...
    if(length(at_index) > 0){
      ...
      #print("got play_df")
      outcome <- all_training[at_index, "outcome"]
      binary_outcome <- ifelse(outcome == 'pass_outcome_caught', 1, 0)
      ...
      tmp_results[play_ix, "outcome"] <- outcome
      tmp_results[play_ix, "binary_outcome"] <- binary_outcome
